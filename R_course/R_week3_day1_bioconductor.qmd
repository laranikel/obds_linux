...
```{r}
Ebox <- "CANNTG"
countPattern("CANNTG", chrY, fixed = FALSE)
...
```{r}
gtf_ACTB <- subset(gtf_data, gtf_data$gene_name == "ACTB")
gtf_ACTB
```
...
--> annotation packages don't update but are available offline and runs a lot faster than annotating with ensemble directly
...
gene symbols are gene names. 

"GO" to include gene ontology/ pathways
library(GO.db) to get the name of the pathwyas

```{r}
AnnotationDbi::select(x = org.Hs.eg.db, keys = human_symbols, columns = c("ENTREZID", "ENSEMBL"), keytype = "SYMBOL" )
```
"GO" to include gene ontology/ pathways
library(GO.db) to get the name of the pathwyas

```{r}
AnnotationDbi::select(x = org.Hs.eg.db, keys = human_symbols, columns = c("ENTREZID", "ENSEMBL", "GO"), keytype = "SYMBOL" )
...
mapID --> one identifier to ONE other identifier; select can output multiple identifier 
...
```{r}
listEnsemblArchives(ENSEMBL_MART_ENSEMBL)
```
